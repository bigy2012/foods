...
if ((isset($_POST['MM_up'])) && ($_POST['MM_up'] == 'form1')) {
        ...
        $sql2 = "UPDATE users SET passwd='$password',fname='$fname', lname='$lname', address='$address', email='$email',phone='$phone', userimg='$img_copy' WHERE uid='$UId'";
        $result = $conn->query($sql2);

    echo "<script>alert('แก้ไขข้อมูลที่มีรูปภาพเรียบร้อยแล้ว ? ');</script>";
        }else{
    $sql2 = "UPDATE users SET passwd='$password',fname='$fname', lname='$lname', address='$address', email='$email',phone='$phone' WHERE uid='$UId'";
    $result = $conn->query($sql2);    
